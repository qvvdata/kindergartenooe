...
write.csv(countup, file ="output/datawrapper/laengerkurzer.csv")
        ...
        #summarise(sum2= length(diffmin[diffmin<0]), sum4 = sum(length[diffmin >0]), na.rm=TRUE)
...
quantile(std$diffmin, na.rm = TRUE, probs = seq(0, 1, 0.1))
...
map_data$diffminmap[map_data$diffmin <= -6] <- -6
map_data$diffminmap[map_data$diffmin >= 85] <- 85
