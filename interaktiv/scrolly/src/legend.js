...
legend['zero-and-linear'] = function(MAP) {
    var legend = L.control({position: 'topleft'});
    var scale = d3scale.scaleLinear();
    var chartdata = null;


    legend.set_data = function(data) {
        chartdata = data;
        var datavalues  = data.map(
        (d) => MAP.value([d])).filter((d) => d>0);
        datavalues.sort();
        chartdata = datavalues;
        scale = scale.domain([datavalues[0],datavalues[datavalues.length-1]])
            .range(MAP.colorschemes.slice(1));
    }

    legend.onAdd = function (map) {
        var div = L.DomUtil.create('div', 'info legend threshold');
        div.innerHTML += '0';
        var numfmt = MAP.numfmt || ((d) => d);
        var colors = [MAP.colorschemes[0], scale(scale.domain()[0]),
            scale((scale.domain()[1]-scale.domain()[0])/2),
            scale(scale.domain()[1])]
        colors.map((c,i) =>
            div.innerHTML+=`<span style="background: ${c}"></span>`);
        div.innerHTML+=`&nbsp;${numfmt(scale.domain()[1])}`;
        return div;
    };
    ...
    legend.getColor = function(data) {
        ...
        if(!d) {
            return {'fillColor': MAP.colorschemes[0], fillOpacity: 1};
        }
        if(d==0) {
            return {'fillColor': MAP.colorschemes[0], fillOpacity: 1};
        }
        return {'fillColor': scale(d), fillOpacity: 1};
    };
